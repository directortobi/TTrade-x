...
const ReferralPage: React.FC<ReferralPageProps> = ({ user }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [statsData, usersData, earningsData] = await Promise.all([
                ...
                referralService.getReferredUsers(user.auth.id),
    ...
    const handleWithdrawalSubmit = async (e: React.FormEvent) => {
        ...
        if (amount > stats.availableEarnings) {
            setWithdrawalError("Withdrawal amount cannot exceed available earnings.");
            return;
        }
        if (walletAddress.trim().length < 26) { // Basic TRC20 address check
             setWithdrawalError("Please enter a valid USDT (TRC20) wallet address.");
            return;
        }
        ...
        try {
            ...
            setWithdrawalSuccess("Your withdrawal request has been submitted successfully! It will be reviewed by an admin.");
            setWithdrawalAmount('');
            setWalletAddress('');
            fetchData(); // Refresh stats
        } catch(err) {
            setWithdrawalError(err instanceof Error ? err.message : "An unknown error occurred.");
        } finally {
        ...
        return (
            <>
                ...
                <div className="bg-gray-800/50 p-4 sm:p-6 rounded-2xl border border-gray-700">
                    ...
                    <p className="text-gray-400 mb-4 text-sm">Share this unique link. You'll earn a 20% commission on the value of every token package they purchase.</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 bg-gray-800/50 p-6 rounded-2xl border border-gray-700 self-start">
                         ...
                         {withdrawalError && <div className="mb-4"><ErrorAlert message={withdrawalError} /></div>}
    ...
    return (
        <div className="max-w-7xl mx-auto animate-fade-in space-y-8">
            <div>
                ...
                <p className="text-gray-400 mt-1">Earn commission on every token purchase made by users you refer.</p>
            </div>
            {renderMainContent()}
        </div>
    );
