    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                // Safety check: Ensure supabase client exists before calling methods
            ...
            return (
                <div className="min-h-screen bg-gray-900 flex flex-col justify-center items-center p-4 text-center">
                    <div className="bg-gray-800 p-8 rounded-2xl border border-blue-500/30 shadow-2xl max-w-md">
                        ...
                        <button onClick={() => setAuthView('login')} className="w-full py-3 px-4 bg-gray-700 hover:bg-gray-600 text-white rounded-xl transition-colors font-semibold">Back to Login</button>
                    </div>
                </div>
            );
        }
        return authView === 'signup' ? (
            <SignUpPage onSignUp={handleSignUp} onNavigateToLogin={() => setAuthView('login')} error={authError} isLoading={isAuthLoading} />
        ) : (
            <LoginPage onLogin={handleLogin} onNavigateToSignUp={() => setAuthView('signup')} error={authError} isLoading={isAuthLoading} successMessage={successMessage} />
        );
    }
